...
'''
purpose: Compute 2D FS using Modified Bishop Method
Input: slice dimensions, water pressure, external line load, seismic load, weight
...
''' main function - 2D slope stability analysis using modified Bishop method'''
def modified_bishop(filename,tol,iterationNMax):
	import math
	...
	FSguess=ordinary.ordinary_method(filename)
	...
	for surface_num in range(totalSlipSurface):
		...
		while difference>tol:
			...
			for slice in sliceInfo:
				...
				P= (slice[9] +slice[10] - (slice[20]*l - u*math.tan(math.radians(slice[21])))*(math.sin(math.radians(slice[3]))/FSguess[surface_num]))/m_alpha 
				numerator+=(slice[20]*l + (P-u)*math.tan(math.radians(slice[21])))*slice[5]
				sum_Wx+=(slice[9]+slice[10])*slice[7]
				sum_Pf+=slice[6]*P
				sum_kWe+=analysisInfo[7]*slice[9]*slice[8]
				sum_Ld+=slice[14]*slice[16]

			F=numerator/(sum_Wx-sum_Pf+sum_kWe+sum_Aa+sum_Ld)
			'''
			print(iterationN)
			print(F)
			print(FSguess[surface_num])
			'''
...
tol=0.0001
iterationNMax=100
filename='test inputs for analysis.csv'
print (modified_bishop(filename,tol,iterationNMax))
